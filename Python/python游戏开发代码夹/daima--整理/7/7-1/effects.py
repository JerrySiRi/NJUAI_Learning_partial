...
class EffectLayer(ColorLayer):
    ...
    def on_key_press(self, key, modifiers):
        if symbol_string(key) == "T":
            self.do(Twirl(amplitude=5, duration=2))#转动效果

        elif symbol_string(key) == "W":
            self.do(Reverse(Waves(duration=2)))#翻转特效

        elif symbol_string(key) == "F":
            self.do(FlipX3D(duration=1) + FlipY3D(duration=1) + Reverse(FlipY3D(duration=1)))#水平方向翻转的3D效果
