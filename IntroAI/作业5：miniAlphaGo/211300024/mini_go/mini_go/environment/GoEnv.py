import os
os.environ['BOARD_SIZE'] = '5'#给定棋盘大小为5*5
...
class Go(object):#把flattern_board_state设置成false》》可以返回二维棋盘状态编码【此时使用CNN作为策略网络-默认是MLP作为策略网络】
    def __init__(self, flatten_board_state=False, discount_factor=1.0):
        self.__state = go.Position(komi=0.5)
        self.__flatten_state = flatten_board_state
        self.__discount_factor = discount_factor
        N = int(os.environ.get("BOARD_SIZE"))#棋盘的长和宽
        self.__state_size = N ** 2#棋盘的格子总数
        self.__action_size = self.__state_size + 1  # board size and an extra action for "pass"
        #一共最多可行的动作是n*n个+1个（可以当前不下棋，pass掉）
