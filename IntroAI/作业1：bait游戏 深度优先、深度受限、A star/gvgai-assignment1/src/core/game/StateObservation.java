...
public class StateObservation {
    /**是ForwardModel的一个变量：可以申请动作、拿到下一个动作
    ...
    /**构造函数，把StateObservation中的model给初始化成 当前游戏的局面！！！
    ...
    /**【走一步】使用枚举变量的action，把从当前局面的model进行一步操作，到下一个局面
     ...
     * @return true if o has the same components as this.
     */
    public boolean equalPosition(Object o)//可以比较所有类呢，只要这个类是Object的对象即可（所有的都是呢！）
    {
        //First simple object-level checks.
        if(this == o) return true;
        if(!(o instanceof StateObservation)) return false;
        StateObservation other = (StateObservation)o;
        //此时o已经可以得出是StateObservation的对象了。只是创建了一个o的副本
